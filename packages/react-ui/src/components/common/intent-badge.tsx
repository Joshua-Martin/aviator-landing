...
const getIntentIcon = (type: IntentSignalType) => {
  const iconMap: Record<IntentSignalType, React.ComponentType<{ className?: string }>> = {
    website_visit: Globe,
 ...
 */
export const IntentBadge: React.FC<IntentBadgeProps> = ({ type, level, label, className }) => {
  ...
  return (
    <Badge
      ...
      className={cn('flex items-center gap-1 text-xs px-2 py-1', styling.className, className)}
    >
