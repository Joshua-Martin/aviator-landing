 ...
 * ErrorAlert
 *
 * Displays an error alert with a rounded red recess-border and a red error icon.
...
export const ErrorAlert: React.FC<ErrorAlertProps> = ({ message, className }) => {
  ...
  return (
    <div
      role="alert"
      className={`flex items-center w-full rounded-lg recess-border border-red-500 bg-red-50 text-red-700 p-4 gap-3 ${
        className ?? ''
      }`}
    >
